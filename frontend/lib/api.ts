...
export const api = {
  ...
  getSenders: (userId: string) => request<Sender[]>(`/senders?userId=${userId}`),
  createSender: (userId: string) =>
    request<Sender>("/senders", { method: "POST", body: JSON.stringify({ userId }) }),

  getScheduled: () => request<ScheduledEmail[]>("/emails/scheduled"),
  getSent: () => request<SentEmail[]>("/emails/sent"),
  ...
  slackStatus: () => request<{ connected: boolean }>("/slack/status"),
};
